    ...
    public partial class frmPO : Form
    {
        ...
        private void SelectPOByNo(string poNo)
        {
            ...
            foreach (DataGridViewRow row in dgvPO.Rows)
            {
                ...
                {
                    dgvPO.ClearSelection(); row.Selected = true;
        ...
        private void BuildUI()
        {
            ...
            int y = 38;
            AddLabel(panelHeader, "PO No (*):", 10, y); txtPONo = AddTxt(panelHeader, 90, y, 140);
            AddLabel(panelHeader, "Tên dự án:", 240, y); txtProjectName = AddTxt(panelHeader, 320, y, 200);
            AddLabel(panelHeader, "Workorder:", 530, y); txtWorkorderNo = AddTxt(panelHeader, 610, y, 160);
            AddLabel(panelHeader, "MPR No:", 780, y); txtMPRNo = AddTxt(panelHeader, 865, y, 250);

            y += 38;
            AddLabel(panelHeader, "Nhà cung cấp:", 10, y);
            cboSupplier = new ComboBox { Location = new Point(115, y), Size = new Size(280, 25), Font = new Font("Segoe UI", 9), DropDownStyle = ComboBoxStyle.DropDown, AutoCompleteMode = AutoCompleteMode.None };
            ...
            y += 38;
            AddLabel(panelHeader, "Prepared:", 10, y); txtPrepared = AddTxt(panelHeader, 90, y, 140);
            AddLabel(panelHeader, "Reviewed:", 240, y); txtReviewed = AddTxt(panelHeader, 320, y, 140);
            AddLabel(panelHeader, "Agreement:", 470, y); txtAgreement = AddTxt(panelHeader, 555, y, 140);
            AddLabel(panelHeader, "Approved:", 705, y); txtApproved = AddTxt(panelHeader, 785, y, 140);
            ...
            // Nút panel detail — thêm "💾 Lưu chi tiết" ← MỚI
            ...
            using (Graphics g = dgvDetails.CreateGraphics())
            {
                ...
                foreach (DataGridViewColumn col in dgvDetails.Columns)
                {
                    ...
                    foreach (DataGridViewRow row in dgvDetails.Rows)
                    {
                        ...
                        if (!string.IsNullOrEmpty(cellValue)) { SizeF size = g.MeasureString(cellValue, dgvDetails.Font); int cw = (int)Math.Ceiling(size.Width) + 15; if (cw > maxWidth) maxWidth = cw; }
                    }
        ...
        {
            var cb = sender as ComboBox; string typedText = cb.Text?.Trim();
            if (string.IsNullOrEmpty(typedText)) { cb.SelectedIndex = 0; return; }
            foreach (var item in cb.Items) { if (item is DataRowView drv) { string value = drv[cb.DisplayMember]?.ToString(); if (value != null && value.Equals(typedText, StringComparison.OrdinalIgnoreCase)) { cb.SelectedItem = item; return; } } }
            cb.SelectedIndex = 0;
        }
        ...
        private void CboSupplier_TextChanged(object sender, EventArgs e)
        {
            ...
            foreach (System.Data.DataRow row in _supplierTable.Rows) { string name = row["Name"].ToString(); if (RemoveDiacritics(name).ToLower().Contains(kn) || name.ToLower().Contains(keyword.ToLower())) filtered.Rows.Add(row["ID"], row["Name"]); }
            if (filtered.Rows.Count == 0) { var empty = new System.Data.DataTable(); empty.Columns.Add("ID", typeof(int)); empty.Columns.Add("Name", typeof(string)); empty.Rows.Add(0, "-- Không tìm thấy --"); BindSupplierCombo(empty); }
            else BindSupplierCombo(filtered);
            _isSearching = true; cboSupplier.Text = keyword; cboSupplier.SelectionStart = keyword.Length; cboSupplier.DroppedDown = true; _isSearching = false;
        }

        private string RemoveDiacritics(string text)
        {
            if (string.IsNullOrEmpty(text)) return "";
            try { string n = text.Normalize(System.Text.NormalizationForm.FormD); var sb = new System.Text.StringBuilder(); foreach (char c in n) if (System.Globalization.CharUnicodeInfo.GetUnicodeCategory(c) != System.Globalization.UnicodeCategory.NonSpacingMark) sb.Append(c); return sb.ToString().Normalize(System.Text.NormalizationForm.FormC); }
            catch { return text; }
        }
        ...
        private void CboSupplier_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter || e.KeyCode == Keys.Tab)
            {
                ...
                {
                    if (cboSupplier.SelectedIndex >= 0) { int sid = Convert.ToInt32(cboSupplier.SelectedValue ?? 0); if (sid > 0) { cboSupplier.DroppedDown = false; _isSearching = true; BindSupplierCombo(_supplierTable); cboSupplier.SelectedValue = sid; _isSearching = false; cboSupplier.BackColor = Color.White; e.SuppressKeyPress = true; e.Handled = true; return; } }
                    string kw = cboSupplier.Text.Trim(); string kwn = RemoveDiacritics(kw).ToLower(); int matchId = 0;
                    foreach (System.Data.DataRowView drv in cboSupplier.Items) { string name = drv["Name"].ToString(); int id = Convert.ToInt32(drv["ID"]); if (id > 0 && (RemoveDiacritics(name).ToLower().Contains(kwn) || name.ToLower().Contains(kw.ToLower()))) { matchId = id; break; } }
                    if (matchId > 0) { cboSupplier.DroppedDown = false; _isSearching = true; BindSupplierCombo(_supplierTable); cboSupplier.SelectedValue = matchId; _isSearching = false; cboSupplier.BackColor = Color.White; }
                    else cboSupplier.BackColor = Color.FromArgb(255, 230, 230);
                }
                e.SuppressKeyPress = true; e.Handled = true;
            }
            if (e.KeyCode == Keys.Escape) { _isSearching = true; BindSupplierCombo(_supplierTable); cboSupplier.Text = ""; cboSupplier.DroppedDown = false; cboSupplier.BackColor = Color.White; _isSearching = false; }
        }
        ...
        {
            if (_isSearching) return; if (cboSupplier.SelectedValue == null) return;
            int supplierId = Convert.ToInt32(cboSupplier.SelectedValue);
            if (supplierId == 0) { cboSupplier.BackColor = Color.White; return; }
            try { cboSupplier.BackColor = Color.White; _isSearching = true; BindSupplierCombo(_supplierTable); cboSupplier.SelectedValue = supplierId; _isSearching = false; } catch { }
        }
        ...
        private void BtnExport_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                var po = _poList.Find(p => p.PO_ID == _selectedPO_ID); var details = _service.GetDetails(_selectedPO_ID); if (po == null) return;
                var suppliers = new SupplierService().GetAll(); var supplier = suppliers.Find(s => s.Supplier_ID == Convert.ToInt32(cboSupplier.SelectedValue?.ToString() ?? "0"));
                var projects = new ProjectService().GetAll(); var project = projects.Find(p => p.WorkorderNo == po.WorkorderNo);
                ...
                using (var package = new OfficeOpenXml.ExcelPackage(new FileInfo(saveDialog.FileName)))
                {
                    ...
                    for (int i = 0; i < detailCount; i++)
                    {
                        ...
                        decimal q = d.Qty_Per_Sheet; decimal wk = d.Weight_kg; decimal realPrice = d.Price; string rem = d.Remarks ?? "";
                        ...
                        ws.Cells[row, 7].Value = d.Qty_Per_Sheet; ws.Cells[row, 8].Value = d.UNIT ?? ""; ws.Cells[row, 9].Value = d.Weight_kg;
                        ws.Cells[row, 10].Value = d.MPSNo ?? ""; ws.Cells[row, 11].Value = d.RequestDay; ws.Cells[row, 12].Value = "Kho DLHI";
                        ws.Cells[row, 13].Value = Math.Round(realPrice, 0); ws.Cells[row, 14].Value = d.Amount; ws.Cells[row, 16].Value = rem;
                        if (i > 0)
                        {
                            for (int col = 1; col <= 16; col++)
                            {
                                ws.Cells[row, col].Style.Border.Top.Style = OfficeOpenXml.Style.ExcelBorderStyle.Thin; ws.Cells[row, col].Style.Border.Bottom.Style = OfficeOpenXml.Style.ExcelBorderStyle.Thin;
                    ...
                    int subTotalRow = startRow + detailCount; int vatRow = subTotalRow + 1;
                    ...
                    ws.Cells[vatRow, 3].Value = "Final Price Requested (Included 10% VAT)"; ws.Cells[vatRow, 14].Formula = $"=N{subTotalRow}*1.1";
        ...
        private TextBox AddTxt(Panel p, int x, int y, int width) { var txt = new TextBox { Location = new Point(x, y), Size = new Size(width, 25), Font = new Font("Segoe UI", 9) }; p.Controls.Add(txt); return txt; }
        ...
        {
            int w = this.ClientSize.Width - 20; int h = this.ClientSize.Height;
            panelTop.Width = w; panelHeader.Width = w; panelDetail.Width = w;
            panelHeader.Top = panelTop.Bottom + 10; panelDetail.Top = panelHeader.Bottom + 10; panelDetail.Height = h - panelDetail.Top - 10;
            dgvPO.Width = panelTop.Width - 20; dgvDetails.Width = panelDetail.Width - 20; dgvDetails.Height = panelDetail.Height - 80;
            txtNotes.Width = panelHeader.Width - txtNotes.Left - 20;
        }

        private void LoadPO()
        {
            try { _poList = _service.GetAll(); BindPOGrid(_poList); lblStatus.Text = $"Tổng: {_poList.Count} đơn PO"; }
            catch (Exception ex) { MessageBox.Show("Lỗi tải dữ liệu: " + ex.Message, "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error); }
        }

        // =========================================================================
        // BIND PO GRID — sắp xếp A-Z theo PO No ← THAY ĐỔI
        // =========================================================================
        private void BindPOGrid(List<POHead> list)
        {
            var suppliers = new SupplierService().GetAll();

            // Sắp xếp A-Z theo PO No trước khi bind
        ...
        private void LoadDetails(int poId)
        {
            try
            {
                ...
                {
                    int idx = dgvDetails.Rows.Add(); var row = dgvDetails.Rows[idx];
                    string remarks = d.Remarks ?? ""; string calcMethod = "Theo SL"; decimal realPrice = d.Price; decimal q = d.Qty_Per_Sheet; decimal wk = d.Weight_kg;
                    if (remarks.Contains("[CALC:KG]")) { calcMethod = "Theo KG"; remarks = remarks.Replace("[CALC:KG]", "").Trim(); if (wk > 0 && q > 0) realPrice = Math.Round((d.Price * q) / wk, 2); }
                    else if (remarks.Contains("[CALC:SL]")) { calcMethod = "Theo SL"; remarks = remarks.Replace("[CALC:SL]", "").Trim(); }
                    else { decimal amtByKG = wk * d.Price * (1 + d.VAT / 100); decimal amtBySL = q * d.Price * (1 + d.VAT / 100); if (Math.Abs(d.Amount - amtByKG) < Math.Abs(d.Amount - amtBySL)) calcMethod = "Theo KG"; }
                    row.Cells["PO_Detail_ID"].Value = d.PO_Detail_ID; row.Cells["MPR_Detail_ID"].Value = d.MPR_Detail_ID.HasValue ? (object)d.MPR_Detail_ID.Value : DBNull.Value;
                    row.Cells["Item_No"].Value = d.Item_No; row.Cells["Item_Name"].Value = d.Item_Name; row.Cells["Material"].Value = d.Material;
                    row.Cells["Asize"].Value = d.Asize; row.Cells["Bsize"].Value = d.Bsize; row.Cells["Csize"].Value = d.Csize;
                    row.Cells["Qty"].Value = d.Qty_Per_Sheet; row.Cells["UNIT"].Value = d.UNIT; row.Cells["Weight"].Value = d.Weight_kg;
                    row.Cells["Price"].Value = realPrice; row.Cells["VAT"].Value = d.VAT; row.Cells["Amount"].Value = d.Amount;
                    row.Cells["Received"].Value = d.Received; row.Cells["MPSNo"].Value = d.MPSNo; row.Cells["DeliveryLocation"].Value = d.DeliveryLocation;
                    row.Cells["Remarks"].Value = remarks; row.Cells["Calc_Method"].Value = calcMethod; row.Cells["Ordered_PO"].Value = "";
                }
                dgvDetails.CellValueChanged += DgvDetails_CellValueChanged;
                UpdateTotal(); AutoAdjustColumnWidths();
            }
            catch (Exception ex) { MessageBox.Show("Lỗi tải chi tiết PO: " + ex.Message, "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error); }
        }

        private void UpdateTotal()
        {
            decimal total = 0; decimal subTotal = 0;
            foreach (DataGridViewRow row in dgvDetails.Rows)
            {
                if (row.IsNewRow) continue;
        ...
        private void DgvDetails_KeyDown(object sender, KeyEventArgs e) { if (e.Control && e.KeyCode == Keys.V) { PasteFromExcel(); e.Handled = true; } }

        private void PasteFromExcel()
        {
            try
            {
                string copiedData = Clipboard.GetText(); if (string.IsNullOrEmpty(copiedData)) return;
                ...
                foreach (string line in lines)
                {
                    ...
                    {
                        int nextItem = dgvDetails.Rows.Count + 1; int newIdx = dgvDetails.Rows.Add(); var r = dgvDetails.Rows[newIdx];
                        r.Cells["Item_No"].Value = nextItem; r.Cells["PO_Detail_ID"].Value = 0; r.Cells["UNIT"].Value = "PCS";
                        r.Cells["Qty"].Value = 0; r.Cells["Weight"].Value = 0; r.Cells["Price"].Value = 0; r.Cells["VAT"].Value = 0;
                        r.Cells["Amount"].Value = 0; r.Cells["Received"].Value = 0; r.Cells["Calc_Method"].Value = "Theo SL"; r.Cells["Ordered_PO"].Value = "";
                    }
                    ...
                    RecalculateAmount(startRow); startRow++;
                }
                AutoAdjustColumnWidths();
            }
            catch (Exception ex) { MessageBox.Show("Lỗi khi dán dữ liệu: " + ex.Message, "Lỗi Paste", MessageBoxButtons.OK, MessageBoxIcon.Error); }
        }
        ...
        private void DgvDetails_CellEndEdit(object sender, DataGridViewCellEventArgs e) { RecalculateAmount(e.RowIndex); AutoAdjustColumnWidths(); }
        ...
        private void BtnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else { var result = _service.Search(txtSearch.Text.Trim()); BindPOGrid(result); lblStatus.Text = $"Tìm thấy: {result.Count} đơn PO"; }
            }
        ...
        private void DgvPO_SelectionChanged(object sender, EventArgs e)
        {
            ...
            txtPrepared.Text = h.Prepared; txtReviewed.Text = h.Reviewed; txtAgreement.Text = h.Agreement; txtApproved.Text = h.Approved;
        ...
        private void BtnNewPO_Click(object sender, EventArgs e) { ClearHeader(); _selectedPO_ID = 0; dgvDetails.Rows.Clear(); UpdateTotal(); txtPONo.Focus(); lblStatus.Text = "Đang tạo đơn PO mới..."; }
        ...
        {
            dgvDetails.EndEdit();
            if (string.IsNullOrWhiteSpace(txtPONo.Text)) { MessageBox.Show("Vui lòng nhập PO No!", "Thiếu thông tin", MessageBoxButtons.OK, MessageBoxIcon.Warning); txtPONo.Focus(); return; }
            ...
            try
            {
                string basePONo = txtPONo.Text.Trim(); int revIdx = basePONo.LastIndexOf("_Rev"); if (revIdx > 0) basePONo = basePONo.Substring(0, revIdx);
                string finalPONo = basePONo; bool isBaseDuplicate = _poList.Exists(p => p.PONo == basePONo && p.PO_ID != _selectedPO_ID);
                if (isBaseDuplicate || nudRevise.Value > 0)
                {
                    if (nudRevise.Value == 0) { MessageBox.Show("Số PO này đã tồn tại!\nVui lòng tăng số Revise để tạo bản sửa đổi.", "Trùng lặp", MessageBoxButtons.OK, MessageBoxIcon.Warning); nudRevise.Focus(); return; }
                    finalPONo = $"{basePONo}_Rev{nudRevise.Value}";
                    if (_poList.Exists(p => p.PONo == finalPONo && p.PO_ID != _selectedPO_ID)) { MessageBox.Show($"Bản '{finalPONo}' cũng đã tồn tại!\nVui lòng tăng Revise lên mức cao hơn.", "Trùng lặp", MessageBoxButtons.OK, MessageBoxIcon.Warning); nudRevise.Focus(); return; }
                }
                var h = new POHead
                {
                ...
                LoadPO(); LoadDetails(_selectedPO_ID);
            }
            catch (Exception ex) { MessageBox.Show("Lỗi khi lưu PO: " + ex.Message, "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error); }
        }

        // =========================================================================
        // LƯU CHI TIẾT — chỉ lưu detail, không đụng Header ← MỚI
        // =========================================================================
        private void BtnSaveDetail_Click(object sender, EventArgs e)
        {
            dgvDetails.EndEdit();
            if (_selectedPO_ID == 0) { MessageBox.Show("Vui lòng chọn hoặc lưu Header PO trước!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }
            if (dgvDetails.Rows.Count == 0) { MessageBox.Show("Không có dòng nào để lưu!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }
            try
            {
                SaveDetailsToDb();
            ...
            catch (Exception ex) { MessageBox.Show("Lỗi khi lưu chi tiết: " + ex.Message, "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error); }
        }
        ...
        {
            if (dgvPO.SelectedRows.Count == 0 || _selectedPO_ID == 0) { MessageBox.Show("Vui lòng chọn một đơn PO trong 'Danh sách đơn đặt hàng' để xóa!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }
            ...
            {
                try { _service.DeletePO(_selectedPO_ID); MessageBox.Show($"Đã xóa thành công đơn PO '{poNo}'!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information); ClearHeader(); dgvDetails.Rows.Clear(); UpdateTotal(); _selectedPO_ID = 0; LoadPO(); }
                catch (Exception ex) { MessageBox.Show("Lỗi khi xóa PO: " + ex.Message, "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error); }
            }
        }
        ...
        private void BtnAddDetail_Click(object sender, EventArgs e)
        {
            ...
            r.Cells["Qty"].Value = 0; r.Cells["UNIT"].Value = "PCS"; r.Cells["Weight"].Value = 0; r.Cells["Price"].Value = 0; r.Cells["VAT"].Value = 0;
        ...
        {
            if (dgvDetails.SelectedRows.Count == 0) { MessageBox.Show("Vui lòng chọn ít nhất một dòng trong danh sách vật tư để xóa!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }
            string msg = dgvDetails.SelectedRows.Count == 1 ? "Bạn có chắc chắn muốn xóa dòng này?" : $"Bạn có chắc chắn muốn xóa {dgvDetails.SelectedRows.Count} dòng đã chọn?";
            if (MessageBox.Show(msg, "Xác nhận xóa", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    ...
                    int itemNo = 1; foreach (DataGridViewRow row in dgvDetails.Rows) if (!row.IsNewRow) row.Cells["Item_No"].Value = itemNo++;
                    UpdateTotal(); AutoAdjustColumnWidths();
                }
                catch (Exception ex) { MessageBox.Show("Lỗi khi xóa: " + ex.Message, "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error); }
            }
        }

        private Dictionary<int, string> GetPoMappingForMpr(int mprId)
        {
            var dict = new Dictionary<int, string>(); if (mprId <= 0) return dict;
            try
            {
                using (var conn = MPR_Managerment.Helpers.DatabaseHelper.GetConnection())
                {
                    ...
                    using (var cmd = new Microsoft.Data.SqlClient.SqlCommand(sql, conn))
                    {
                        ...
                        using (var reader = cmd.ExecuteReader()) { while (reader.Read()) { if (reader["MPR_Detail_ID"] != DBNull.Value) { int detailId = Convert.ToInt32(reader["MPR_Detail_ID"]); string poNo = reader["PONo"]?.ToString() ?? ""; if (dict.ContainsKey(detailId)) { if (!dict[detailId].Contains(poNo)) dict[detailId] += ", " + poNo; } else dict[detailId] = poNo; } } }
                    }
                }
            }
            catch (Exception ex) { System.Diagnostics.Debug.WriteLine("Lỗi lấy PO Mapping: " + ex.Message); }
            return dict;
        }
        ...
        private void BtnImportMPR_Click(object sender, EventArgs e)
        {
            using (var dlg = new frmSelectMPR())
            {
                ...
                {
                    ClearHeader(); _selectedPO_ID = 0; _details.Clear(); dgvDetails.Rows.Clear();
                    var mpr = dlg.SelectedMPR; var details = dlg.SelectedDetails; var poMapping = GetPoMappingForMpr(mpr.MPR_ID);
                    ...
                    try
                    {
                        ...
                        if (project != null) { txtWorkorderNo.Text = project.WorkorderNo ?? ""; _projectCodeImport = project.ProjectCode; txtPONo.Text = GenerateAutoPoNo(project.POCode ?? project.ProjectCode ?? ""); }
                    ...
                    foreach (var d in details)
                    {
                        ...
                        string aSize = d.Thickness_mm > 0 ? d.Thickness_mm.ToString() : (d.Depth_mm > 0 ? d.Depth_mm.ToString() : ""); string bSize = d.C_Width_mm > 0 ? d.C_Width_mm.ToString() : "";
        ...
        public void ImportMPRByNo(string mprNo)
        {
            ...
            try
            {
                ...
                if (mpr == null) { MessageBox.Show($"Không tìm thấy MPR: {mprNo}", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }
                var details = mprService.GetDetails(mpr.MPR_ID);
                if (details == null || details.Count == 0) { MessageBox.Show($"MPR {mprNo} chưa có chi tiết vật tư!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }
                ClearHeader(); _selectedPO_ID = 0; _details.Clear(); dgvDetails.Rows.Clear();
            ...
            catch (Exception ex) { MessageBox.Show("Lỗi import MPR: " + ex.Message, "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error); }
        }
        ...
        private void BtnClearHeader_Click(object sender, EventArgs e) { ClearHeader(); dgvDetails.Rows.Clear(); UpdateTotal(); _selectedPO_ID = 0; LoadPO(); }

        private void ClearHeader()
        {
            txtPONo.Text = ""; txtProjectName.Text = ""; txtWorkorderNo.Text = ""; txtMPRNo.Text = "";
            txtPrepared.Text = ""; txtReviewed.Text = ""; txtAgreement.Text = ""; txtApproved.Text = ""; txtNotes.Text = "";
