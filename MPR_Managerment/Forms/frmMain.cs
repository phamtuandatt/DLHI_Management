...
namespace MPR_Managerment.Forms
{
    ...
    public partial class frmMain : Form
    {
        ...
        private void BuildHeader()
        {
            ...
            _btnConfigAI.FlatAppearance.BorderSize = 0;
            _btnConfigAI.FlatAppearance.MouseOverBackColor = Color.FromArgb(80, 40, 150);
            new ToolTip().SetToolTip(_btnConfigAI, "Cấu hình AI (API Key, Model...)");
            _btnConfigAI.Location = new Point(panelHeader.Width - 635, 12);
            _btnConfigAI.Click += (s, e) => ShowConfigAIDialog();
            panelHeader.Controls.Add(_btnConfigAI);
            ...
            panelHeader.Resize += (s, ev) =>
            {
                ...
                _btnConfigAI.Left = panelHeader.Width - 635;
            };
        ...
        // =====================================================================
        private void ShowConfigAIDialog()
        {
            // Đọc cấu hình AI hiện tại từ file (nếu có)
            ...
            string currentRouterApiUrl = "http://localhost:20128/v1/chat/completions";
            string currentRouterModel = "ForVSCode";
            ...
            if (System.IO.File.Exists(configPath))
            {
                try
                {
                    ...
                    if (doc.RootElement.TryGetProperty("ApiKey", out var ak))
                        currentApiKey = ak.GetString() ?? "";
                    if (doc.RootElement.TryGetProperty("Model", out var md))
                        currentModel = md.GetString() ?? "gemini-2.0-flash";
                    if (doc.RootElement.TryGetProperty("Endpoint", out var ep))
                        currentEndpoint = ep.GetString() ?? "";
                    if (doc.RootElement.TryGetProperty("RouterApiUrl", out var rau))
                        currentRouterApiUrl = rau.GetString() ?? "http://localhost:20128/v1/chat/completions";
                    if (doc.RootElement.TryGetProperty("RouterModel", out var rm))
                        currentRouterModel = rm.GetString() ?? "ForVSCode";
                }
                catch { }
            }
            ...
            pHeader.Controls.Add(new Label
            {
                Text = "🤖  Cấu hình AI — API Key & Model",
                Font = new Font("Segoe UI", 12, FontStyle.Bold),
                ForeColor = Color.White,
                Location = new Point(16, 13),
                Size = new Size(460, 26)
            });
            dlg.Controls.Add(pHeader);
            ...
            dlg.Controls.Add(txtApiKey);

            // Toggle hiển thị API Key
            var chkShow = new CheckBox
            {
                Text = "Hiện",
                Location = new Point(100, y + 28),
                Size = new Size(80, 20),
                Font = new Font("Segoe UI", 8)
            };
            chkShow.CheckedChanged += (s, e) => txtApiKey.UseSystemPasswordChar = !chkShow.Checked;
            ...
            var txtEndpoint = new TextBox
            {
                ...
                PlaceholderText = "(Tùy chọn) Custom API endpoint",
                Font = new Font("Segoe UI", 9)
            };
            ...
            var txtRouterApiUrl = new TextBox
            {
                Location = new Point(100, y),
                Size = new Size(380, 24),
                Text = currentRouterApiUrl,
                PlaceholderText = "http://localhost:20128/v1/chat/completions",
                Font = new Font("Segoe UI", 9)
            };
            dlg.Controls.Add(txtRouterApiUrl);
            y += 38;
            ...
            var txtRouterModel = new TextBox
            {
                Location = new Point(110, y),
                Size = new Size(370, 24),
                Text = currentRouterModel,
                PlaceholderText = "ForVSCode",
                Font = new Font("Segoe UI", 9)
            };
            dlg.Controls.Add(txtRouterModel);
            y += 42;
            ...
            btnSave.Click += (s, e) =>
            {
                try
                {
                    var config = new
                    {
                        ...
                        RouterApiUrl = txtRouterApiUrl.Text.Trim(),
                        RouterModel = txtRouterModel.Text.Trim()
                    };
                    string json = System.Text.Json.JsonSerializer.Serialize(config,
                        new System.Text.Json.JsonSerializerOptions { WriteIndented = true });
                    System.IO.File.WriteAllText(configPath, json);
                    lblStatus.ForeColor = Color.Green;
                    lblStatus.Text = "✅ Đã lưu cấu hình AI thành công!";
                }
                catch (Exception ex)
                {
            ...
            dlg.ClientSize = new Size(500, y + 50);
            dlg.ShowDialog(this);
        }
