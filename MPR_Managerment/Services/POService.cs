...
IF NOT EXISTS (SELECT 1 FROM sys.columns
    ...
    ALTER TABLE PO_head ADD Email_Sent_At DATETIME NULL;", conn).ExecuteNonQuery();
        ...
        public List<PODetail> GetDetails(int poId)
        {
            ...
            {
                conn.Open();
                var cmd = new SqlCommand("SELECT pD.*, mD.ProductCode FROM PO_Detail pD LEFT JOIN MPR_Details mD ON pD.MPR_Detail_ID = mD.Detail_ID  WHERE pD.PO_ID = @id ORDER BY pD.Item_No", conn);
                cmd.Parameters.AddWithValue("@id", poId);
                var r = cmd.ExecuteReader();
        ...
        private PODetail MapDetail(SqlDataReader r)
        {
            return new PODetail
            {
                ...
                ProductCode = r["ProductCode"] != DBNull.Value ? r["ProductCode"].ToString() : ""
            };
        }
    }
}